...
from atomic_status import detail_mapping
 ...
 while True:
  try:
   ...
   for c in s.get('completed_steps',[]):
    d=detail_mapping(c.get('details'))
  ...
  time.sleep(15)
PAGE='''<!doctype html><meta charset="utf-8"><title>D5 â€” Supervision qualitÃ©</title><style>body{background:#101820;color:#edf3fa;font:17px system-ui;max-width:1050px;margin:35px auto;padding:0 20px}.box{background:#1b2936;padding:20px;border-radius:12px;margin:15px 0}td{padding:10px}small{color:#9bb4c9}pre{white-space:pre-wrap;font-size:13px}progress{width:100%;height:20px}</style><h1>D5 â€” Supervision qualitÃ©</h1><small>Collecte ~3 h 02 Â· Source immuable Â· Paper non dÃ©marrÃ© Â· Alerte de durÃ©e sans coupure</small><div class="box" id="main">Chargementâ€¦</div><div class="box" id="telemetry"></div><div class="box"><table id="steps"></table></div><pre id="detail"></pre><script>
...
async function tick(){try{let s=await(await fetch('/status',{cache:'no-store'})).json();main.replaceChildren();line(main,`Ã‰tape ${s.step}/8 â€” ${s.stage}`,'h2');line(main,`${s.status} Â· Reprise ${fmt(s.elapsed_seconds/60)} min Â· Ã‰tape ${fmt(s.stage_elapsed_seconds/60)} min`);line(main,`Sous-Ã©tape : ${s.sql||s.detail?.stage||s.detail?.step||s.stage}`);line(main,`Lignes : ${fmt(s.rows_processed)} / ${fmt(s.total_rows)} Â· ${fmt(s.rows_per_second)} lignes/s`);if(s.percent!=null){let p=document.createElement('progress');p.max=100;p.value=s.percent;main.append(p)}if(s.one_hour_alert)line(main,'ALERTE : plus dâ€™une heure ; le contrÃ´le continue sans interruption.');line(main,`Provenance source : ${s.source_hash_status?.status||'PENDING'}`);if(s.post_review?.phase)line(main,`ComplÃ©ments : ${s.post_review.status} â€” ${s.post_review.phase}`);if(s.hash_progress?.status==='RUNNING')line(main,`SHA source : ${fmt(s.hash_progress.bytes_processed/1e9)} / ${fmt(s.hash_progress.total_bytes/1e9)} Go Â· ${fmt(s.hash_progress.bytes_per_second/1e6)} Mo/s`);if(s.supplement_progress?.status==='RUNNING')line(main,`Timestamps/gaps : ${fmt(s.supplement_progress.rows_processed)} / ${fmt(s.supplement_progress.total_rows)} lignes Â· ${fmt(s.supplement_progress.rows_per_second)} lignes/s`);telemetry.replaceChildren();let p=s.process||{};line(telemetry,`PID ${p.pid} (${s.active_process_role}) Â· Vivant : ${p.alive} Â· CPU cumulÃ© ${fmt(p.cpu_seconds)} s Â· ${fmt(p.cpu_seconds_per_second*100)} % dâ€™un cÅ“ur`);line(telemetry,`Lectures ${fmt(p.read_bytes/1e9)} Go Â· ${fmt(p.read_bytes_per_second/1e6)} Mo/s Â· Ã‰critures ${fmt(p.write_bytes/1e9)} Go`);line(telemetry,`Heartbeat audit : ${s.progress_last_updated_utc} (Ã¢ge ${fmt(s.heartbeat_age_seconds)} s)`);line(telemetry,`RelevÃ© externe : ${s.monitor_updated_utc}`);line(telemetry,'I/O processus inclut le cache OS ; aucune conversion fictive en lignes auditÃ©es.','small');steps.replaceChildren();for(let c of s.completed_steps||[]){let tr=document.createElement('tr');for(let v of [c.step,c.stage,c.display_status||c.status,fmt(c.original_seconds??c.seconds)+' s']){let td=document.createElement('td');td.textContent=v;tr.append(td)}steps.append(tr)}detail.textContent=JSON.stringify({detail:s.detail,error:s.error,monitor_error:s.monitor_error},null,2)}catch(e){main.textContent=String(e)}}tick();setInterval(tick,2000);
</script>'''
