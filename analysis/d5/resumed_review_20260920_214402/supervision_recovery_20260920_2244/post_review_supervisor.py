...
def main(review,reference):
 ...
 def update(phase,**extra):
  status.update(extra,phase=phase,elapsed_seconds=time.monotonic()-start,updated_utc=datetime.datetime.now(datetime.timezone.utc).isoformat());tmp=checkpoint.with_suffix('.tmp');tmp.write_text(json.dumps(status,indent=2));tmp.replace(checkpoint)
