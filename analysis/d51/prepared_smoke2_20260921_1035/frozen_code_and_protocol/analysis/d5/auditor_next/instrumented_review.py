...
NAMES=['Projection m̩tadonn̩es neuve','Ex̩cution audit_next complet','SQLite integrity_check source','SQLite foreign_key_check source','M̩triques qualit̩ / provenance source','Replay NoTrade 1 source','Replay NoTrade 2 source','Comparaison compl̬te et quality gate']
def run(source,out,session,budget=3600,minimum=10800):
 ...
 state={'pid':os.getpid(),'status':'RUNNING','steps_total':8,'research_allowed':False,'paper_started':False,'source':str(source),'minimum_seconds':minimum,'budget_seconds':budget}
 def save(path,data):
 ...
 def pulse(extra=None,force=False):
  now=time.monotonic()
  if now-started>budget:raise TimeoutError('REVIEW_TIME_BUDGET_EXCEEDED')
 ...
 def begin(n):
  current[0]=n;stage_start[0]=time.monotonic();pulse({'step':n,'stage':NAMES[n-1],'rows_processed':None,'total_rows':None,'percent':None,'rows_per_second':None,'detail':None},True)
 ...
 class Connection(sqlite3.Connection):
  def execute(self,sql,parameters=()):
   ...
   if normalized in ('pragma integrity_check','pragma foreign_key_check'):
    number=3 if normalized.endswith('integrity_check') else 4;begin(number);rows=list(super().execute(sql,parameters));okay=rows==[('ok',)] if number==3 else not rows
 ...
 def connect(*args,**kwargs):
  ...
  kwargs['factory']=Connection;c=sqlite3.connect(*args,**kwargs)
 ...
 def replay_call(*args,**kwargs):
  ...
  kwargs['decision_sink']=tracked;result=replay_database(*args,**kwargs);m=result['metrics'];ok=count[0]==total[0] and m['number_of_orders']==0 and m['number_of_fills']==0 and m['final_cash']==500 and not any(m[k] for k in ('paired_qty','directional_up','directional_down'))
 ...
 save(out/'RUN_MANIFEST.json',{'source':str(source),'source_stat_before':before,'new_review_no_old_results':True,'quality_sha256':hashlib.sha256(pathlib.Path(quality.__file__).read_bytes()).hexdigest(),'code_files':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in pathlib.Path(__file__).parent.glob('*.py')},'source_integrity_fk_and_two_replays':True,'old_audit_disposition':'OLD_AUDIT_ABORTED_BY_USER_FOR_PERFORMANCE'})
 try:
  ...
  state.update(status='FAILED',D5_DATA_QUALITY='FAIL',error=repr(exc),elapsed_seconds=time.monotonic()-started,completed_steps=completed);save(out/'progress.json',state);save(out/'VERDICT.json',{'D5_DATA_QUALITY':'FAIL','duration_seconds':time.monotonic()-started,'failures':[repr(exc)],'replay_hashes':'Only completed REPLAY_N.json are evidence; not fabricated for unexecuted steps'});raise
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--db',type=pathlib.Path,required=True);p.add_argument('--out',type=pathlib.Path,required=True);p.add_argument('--session',required=True);p.add_argument('--budget-seconds',type=int,default=3600);a=p.parse_args()
 if process_alive(12408):raise RuntimeError('OLD_AUDIT_STILL_ALIVE')
 run(a.db,a.out,a.session,a.budget_seconds)
