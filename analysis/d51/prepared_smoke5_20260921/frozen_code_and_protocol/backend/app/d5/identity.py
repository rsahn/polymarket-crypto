...
class MarketIdentity:
    ...
    def fields(self):
        # All identity members are immutable scalars; retain field order without
        # recursively copying this frozen dataclass on every market event.
        return dict(market_duration=self.market_duration, market_slug=self.market_slug,
                    condition_id=self.condition_id, token_up=self.token_up,
                    token_down=self.token_down, expiry_ts_ms=self.expiry_ts_ms)
