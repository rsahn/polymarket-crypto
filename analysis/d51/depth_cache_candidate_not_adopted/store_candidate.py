...
class Store:
    ...
    def __init__(self, path, config=None, *, compress_payloads=False):
        ...
        self.depth_cache_hits = 0
        self.depth_cache_misses = 0
    ...
    def pack_depth(self, value):
        # Internal immutable byte key only: never unpickle external data.
        # Distinguish numeric types and signed zero; no mutable input aliasing.
        key = pickle.dumps(value, protocol=5)
        if key in self._depth_cache:
            self.depth_cache_hits += 1
            self._depth_cache.move_to_end(key)
            return self._depth_cache[key]
        self.depth_cache_misses += 1
        packed = self.pack(value)
        self._depth_cache[key] = packed
        if len(self._depth_cache) > 256:
        ...
        for side in ('UP','DOWN'):
            ...
            self.db.execute('INSERT INTO book_sides VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                            (eid,side,q['token_id'],q.get('event_ts_ms'),
                             ...
                             self.pack_depth(q.get('bids',[])),self.pack_depth(q.get('asks',[])),
