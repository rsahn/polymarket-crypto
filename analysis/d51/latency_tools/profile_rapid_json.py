...
import rapid_encoder as encoder
...
out=ROOT/'analysis/d51'/('rapid_json_profile_'+datetime.datetime.now().strftime('%Y%m%d_%H%M%S'));out.mkdir(exist_ok=False)
...
result=dict(candidate_encoder_sha256=hashlib.sha256((ROOT/'analysis/d51/native_json_candidate/rapid_encoder.py').read_bytes()).hexdigest(),rapidjson_version=encoder.rapidjson.__version__,technical_only=True,not_D6=True,not_a_quality_smoke=True,profile_overhead_present=profiling,frames=frames,seconds=elapsed,frames_per_second=frames/elapsed,timing_seconds=timing,stage_counters=stages.snapshot(),capture_span_seconds=(rows[-1][0]-rows[0][0])/1e9,counts=counts,source_hashes=hashes,code_version=code_version())
