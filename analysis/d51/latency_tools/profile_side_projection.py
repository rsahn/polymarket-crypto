...
import importlib.util,cProfile,datetime,hashlib,io,json,pathlib,pstats,sys,time
ROOT=pathlib.Path(__file__).resolve().parents[3];sys.path.insert(0,str(ROOT/'backend'))
spec=importlib.util.spec_from_file_location('candidate_collector',ROOT/'analysis/d51/side_projection_candidate/candidate.py');mod=importlib.util.module_from_spec(spec);spec.loader.exec_module(mod);PolymarketOrderbookCollector=mod.PolymarketOrderbookCollector
...
out=ROOT/'analysis/d51'/('side_projection_profile_'+datetime.datetime.now().strftime('%Y%m%d_%H%M%S'));out.mkdir(exist_ok=False)
