...
sys.path.insert(0,str(ROOT/'analysis/d51/exact_depth_cache_candidate'))
from candidate import CachedStore as Store
...
out=ROOT/'analysis/d51'/('exact_cache_profile_'+datetime.datetime.now().strftime('%Y%m%d_%H%M%S'));out.mkdir(exist_ok=False)
...
result=dict(cache_hits=store.cache_hits,cache_misses=store.cache_misses,candidate_sha256=hashlib.sha256((ROOT/'analysis/d51/exact_depth_cache_candidate/candidate.py').read_bytes()).hexdigest(),technical_only=True,not_D6=True,not_a_quality_smoke=True,profile_overhead_present=profiling,frames=frames,seconds=elapsed,frames_per_second=frames/elapsed,timing_seconds=timing,stage_counters=stages.snapshot(),capture_span_seconds=(rows[-1][0]-rows[0][0])/1e9,counts=counts,source_hashes=hashes,code_version=code_version())
