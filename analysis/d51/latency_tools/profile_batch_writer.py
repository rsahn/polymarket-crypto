...
def main():
    ...
    out=ROOT/'analysis/d51'/('batch_writer_profile_'+datetime.datetime.now().strftime('%Y%m%d_%H%M%S'));out.mkdir(exist_ok=False)
    ...
    rows.sort(key=lambda x:x[0]);ctx=mp.get_context('spawn');channel=ctx.Queue(maxsize=8)
    ...
    def send(item):
        nonlocal batch
        if item is None:
            if batch:enqueue(batch);batch=[]
            enqueue(None)
        else:
            batch.append(item)
            if len(batch)>=32:enqueue(batch);batch=[]
    ...
    result.update(technical_only=True,not_D6=True,not_prospective=True,frames=sent,total_seconds=elapsed,frames_per_second=sent/elapsed,normalize_seconds=normalize_seconds,queue_put_wall_seconds=wait_seconds,queue_capacity=256,batch_size=32,no_dropped_frames=True,drained=True,code_version=code_version(),timing_note='Captured timestamps reused only in technical fixture; not a live latency measurement. Includes process startup and drain.')
