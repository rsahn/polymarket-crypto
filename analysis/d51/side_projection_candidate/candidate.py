...
class PolymarketOrderbookCollector:
    ...
    def __init__(
        ...
        if timestamp_contract not in ("legacy", "D5.1"):
            raise ValueError("Unknown timestamp contract")
        self.timestamp_contract = timestamp_contract
        self.performance = performance
        self._last_source_ts = {}
        self._source_metadata = {}
        self._wire_seen = deque(maxlen=2048)
        self._wire_seen_index = set()
    ...
    def normalize_snapshot(self, payload: Any, received_ts_ms=None) -> Dict[str, Any]:
        ...
        """
        received_ts_ms = received_ts_ms if received_ts_ms is not None else time.time_ns()//1_000_000
        ...
        reject = 'DUPLICATE_EVENT' if wire_digest in self._wire_seen_index else None
        updates = []
        pending_source_ts = dict(self._last_source_ts)
        for raw in events:
            ...
            try:
                ...
                source_ts = None
            for t in tokens:
                prior_ts = pending_source_ts if self.timestamp_contract == 'D5.1' else self._last_source_ts
                if source_ts is not None and source_ts < prior_ts.get(t, -1):
                    reject = 'OUT_OF_ORDER'
                if changes or 'bids' in event or 'asks' in event:
                    ...
                    if self.timestamp_contract == 'D5.1':
                        if source_ts is None:
                            reject = 'MISSING_EVENT_TIMESTAMP'
                        else:
                            pending_source_ts[t] = source_ts
        if reject:
            return {'market_key': self.market_key, 'token_ids': dict(self.token_ids),
                    'recv_ts_ms': received_ts_ms, 'received_ts_ms': received_ts_ms, 'reject_reason': reject,
                    'stale_token': reject == 'CROSS_MARKET_REJECT', 'wire_hash': wire_digest,
                    **(self.identity.fields() if self.identity else {})}
        # Only accepted, distinct messages enter this bounded window.
        if len(self._wire_seen) == self._wire_seen.maxlen:
            self._wire_seen_index.remove(self._wire_seen[0])
        self._wire_seen.append(wire_digest)
        self._wire_seen_index.add(wire_digest)
        ...
        for outcome, token_id in self.token_ids.items():
            ...
            top_object = self._authoritative_top.get(token)
            cached = self._projection_cache.get(token)
            if cached and cached[0] is bids and cached[1] is asks and cached[2] is top_object:
                projection = cached[3]
                values[outcome.lower()] = {
                    **projection,
                    "bids": list(projection["bids"]),
                    "asks": list(projection["asks"]),
            ...
            # Cache only the depth projection. Metadata remains current per message.
            projection = {k: v for k, v in values[outcome.lower()].items()
                          if k in ("token_id", "bid", "ask", "bid_qty", "ask_qty", "bids", "asks")}
            projection["bids"] = list(projection["bids"])
            projection["asks"] = list(projection["asks"])
            self._projection_cache[token] = (bids, asks, top_object, projection)

        temporal = {}
        if self.timestamp_contract == "D5.1":
            # Composite book-state time, not the time of a non-book message.
            # Raw wire metadata is retained separately; no clock correction.
            side_times = [values[side].get("event_ts_ms") for side in ("up", "down")]
            event_ts_ms = max(side_times) if all(t is not None for t in side_times) else None
            temporal = {"timestamp_contract": "D5.1", "wire_event_ts_ms": wire_event_ts_ms,
                        "book_state_source_ts_ms": event_ts_ms}
        return {
            **temporal,
