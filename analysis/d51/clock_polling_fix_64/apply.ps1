...
if ($beforeParameters.Type -ne 'NTP' -or $beforeParameters.NtpServer -ne 'time.windows.com,0x8' -or $beforeConfig.MinPollInterval -ne 6 -or $beforeConfig.MaxPollInterval -ne 9) { throw 'Unexpected configuration; no changes applied' }
...
try {
 ...
 Set-ItemProperty -LiteralPath $configKey -Name MaxPollInterval -Value 6
 ...
 $rollback=& "$env:SystemRoot\System32\w32tm.exe" /config /manualpeerlist:time.windows.com,0x8 /syncfromflags:manual /update 2>&1
