...
def supplement(db,session,started,stopped,progress=None):
    ...
    def add_gap(feed,kind,a,b,verified=False,reconnected=False):
        ...
        if elapsed>5000 or kind in ('STARTUP_GAP','SHUTDOWN_GAP') or failure:
    ...
    for row in db.execute('''SELECT e.*,u.event_ts_ms AS up_source,u.received_ts_ms AS up_receive,
        ...
        if kind=='RECONNECT':reconnects.add(row['market_duration'])
        ...
        if expected is None or row['event_ts_ms']!=expected or p.get('book_state_source_ts_ms')!=expected:failures['BOOK_STATE_TIMESTAMP_MISMATCH']+=1
